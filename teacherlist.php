...
if (!empty($user_id)) {
    ...
    if (isset($_GET['id']) && !empty($_GET['id'])) {
        ...
        if ($valueInsert > 0) {
            header("location:teacherlist.php?status=success");
        } else {
            $error = "Teacher has not been deleted.";
        }
    }
    ?>
    <html>
        ...
        <body>
            ...
            <header id="head" class="secondary">
                <div class="container">
                    <div class="row">
                        <div class="col-sm-8">
                            <h1 style ="color: #800000;">Teacher List</h1>
                        </div>
                    </div>
                </div>
            </header>            
                    ...
                    <div class="col-md-8">
                        ...
                        <table class="table_list">
                            ...
                            if (isset($_GET['status']) && $_GET['status'] == "success") {
                                echo '<tr><td colspan="4">Teacher has been successfully deleted.</td></tr>';
                            }
                            if (!empty($error)) {
                            ...
                            <?php
                            $i = 0;
                            $sql = "SELECT * FROM users where user_type = 'teacher' ORDER BY name ASC";
                            $result = mysql_query($sql);
                            if (mysql_num_rows($result) > 0) {
                                while ($row = mysql_fetch_array($result)) {
                                    ...
                                    <tr>
                                        ...
                                        <td class="grid_label"><a href="teacherlist.php?id=<?php echo $row ['id']; ?>">Delete</a></td>                            
                                    </tr>
                                    <?php
                                }
                                ?>
